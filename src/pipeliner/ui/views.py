    ...
    <html lang=\"zh-CN\">
      <head>
        ...
        <style>
          ...
          .badge {{ display: inline-block; padding: 2px 8px; border-radius: 999px; background: #e5e7eb; }}
          .attention {{ background: #fef3c7; }}
          pre {{ background: #111827; color: white; padding: 12px; border-radius: 6px; overflow-x: auto; }}
...
def render_index(attention_runs: list[dict[str, Any]]) -> str:
    rows = [[item["run_id"], item["workflow_id"], item["status"], item.get("stop_reason", "")] for item in attention_runs]
    table = _table(["Run", "Workflow", "Status", "Stop Reason"], rows) if rows else "<p>当前没有需要人工介入的运行。</p>"
    body = f"""
    <h1>Pipeliner MVP</h1>
...
def render_workflow_view(workflow: dict[str, Any]) -> str:
    ...
    rows = [[node["node_id"], node["title"], ", ".join(node["depends_on"]), node["executor"]["skill"]] for node in nodes]
    body = f"""
...
def render_run_view(detail: dict[str, Any], callbacks: list[dict[str, Any]]) -> str:
    ...
    node_rows = [
        [node["node_id"], node["round_no"], node["status"], node.get("waiting_for_role", ""), node.get("stop_reason", "")]
        for node in nodes
    ]
    artifact_rows = [
    ...
    callback_rows = [
        [item["event_id"], item["node_id"], item["round_no"], item["actor_role"], item.get("verdict_status", "")]
        for item in callbacks
    ]
    body = f"""
    <h1>Run {escape(run['id'])}</h1>
    <p>
      <span class=\"badge {'attention' if run['status'] == 'needs_attention' else ''}\">{escape(run['status'])}</span>
      Workflow: <code>{escape(detail['workflow']['workflow_id'])}@{escape(detail['workflow']['version'])}</code>
    </p>
    <p>Workspace: <code>{escape(run['workspace_root'])}</code></p>
    <h2>Nodes</h2>
    {_table(["Node", "Round", "Status", "Waiting For", "Stop Reason"], node_rows)}
    <h2>Artifacts</h2>
    {_table(["Artifact", "Version", "Kind", "Storage URI"], artifact_rows) if artifact_rows else '<p>暂无 artifact。</p>'}
    <h2>Callbacks</h2>
    {_table(["Event", "Node", "Round", "Actor", "Verdict"], callback_rows) if callback_rows else '<p>暂无 callback。</p>'}
    """
