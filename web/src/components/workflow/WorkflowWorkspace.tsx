...
interface WorkflowWorkspaceProps {
  ...
  defaultTab?: 'graph' | 'cards' | 'inputs' | 'spec' | 'lint';
}
...
export function WorkflowWorkspace({
  ...
  const graphTab = t('tabs.graph');
  const cardsTab = t('tabs.cards');
  const specTab = t('tabs.spec');
  ...
  const defaultTabLabel = tabLabelByKey[defaultTab];
  const [activeTab, setActiveTab] = useState(defaultTabLabel);
  const [selectedNodeData, setSelectedNodeData] = useState<unknown>(null);
  const effectiveActiveTab = tabs.includes(activeTab) ? activeTab : defaultTabLabel;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      <div className="px-4">
        <TabList tabs={tabs} activeTab={effectiveActiveTab} onTabChange={setActiveTab} />
      </div>

      <div className="relative flex-1 overflow-hidden bg-stone-50">
        {effectiveActiveTab === graphTab && (
          nodes.length > 0 ? (
        ...
        {effectiveActiveTab === cardsTab && (
          <div className="grid h-full gap-4 overflow-auto p-4 xl:grid-cols-2">
            {displayCards.length === 0 ? (
              <div className="col-span-full flex min-h-[18rem] items-center justify-center rounded-3xl border border-dashed border-stone-300 bg-white px-6 text-sm text-stone-500">
                {t('graphEmpty')}
              </div>
            ) : (
              displayCards.map((card) => (
                <button
                  key={`${card.node_id || 'node'}-${card.title || 'untitled'}`}
                  type="button"
        ...
        {inputEditor && inputTab && effectiveActiveTab === inputTab && (
          <div className="h-full overflow-auto p-4">
            {inputEditor}
          </div>
        )}

        {effectiveActiveTab === specTab && <WorkflowSpecView spec={spec} />}

        {effectiveActiveTab === lintTab && (
